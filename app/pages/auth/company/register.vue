...
import { type CompanySignUpFormValues, companySignUpFormSchema } from "@/types";

const router = useRouter();
const companyAuthStore = useCompanyAuthStore();

onMounted(() => {
  if (companyAuthStore.isAuthenticated) {
    router.push("/company");
    return;
  }
});

const { handleSubmit, meta, isSubmitting } = useForm<CompanySignUpFormValues>({
  validationSchema: toTypedSchema(companySignUpFormSchema),
  initialValues: {
    email: "",
    name: "",
    password: "",
...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    await companyAuthStore.register({
      email: values.email,
      name: values.name,
